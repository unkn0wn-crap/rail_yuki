...
yuki = [
    [
        InlineKeyboardButton(text="ɴᴏᴛ ᴀᴠᴀɪʟᴀʙʟᴇ",),
...
DONATE_STRING = f"""ʜᴇʏ ʜᴜᴍᴀɴ, ʜᴀᴩᴩʏ ᴛᴏ ʜᴇᴀʀ ᴛʜᴀᴛ ʏᴏᴜ ᴡᴀɴɴᴀ ᴅᴏɴᴀᴛᴇ. ʏᴏᴜ ᴄᴀɴ ᴅɪʀᴇᴄᴛʟʏ ᴄᴏɴᴛᴀᴄᴛ ᴍʏ ᴅᴇᴠᴇʟᴏᴘᴇʀ @ɢʜᴏsᴛ_ᴋᴜɴ ғᴏʀ ᴅᴏɴᴀᴛɪɴɢ ᴏʀ ʏᴏᴜ ᴄᴀɴ ᴠɪsɪᴛ ᴍʏ sᴜᴩᴩᴏʀᴛ ᴄʜᴀᴛ @corpsealone | @ghost_kun ᴀɴᴅ ᴀsᴋ ᴛʜᴇʀᴇ ᴀʙᴏᴜᴛ ᴅᴏɴᴀᴛɪᴏɴ"""
...
def yuki_about_callback(update: Update, context: CallbackContext):
    ...
    if query.data == "yuki_":
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_caption(f"ϟ ɪ ᴀᴍ {dispatcher.bot.first_name} "
            "\n\n⎋ Oᴡɴᴇʀ : @corpsealone | @ghost_kun",
            "\n\n⎋  「ᴘʌʀʌᴅᴏx」",
            parse_mode=ParseMode.MARKDOWN,
...
def get_help(update: Update, context: CallbackContext):
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 2 and any(args[1].lower() == x for x in HELPABLE):
            ...
            return
        update.effective_message.reply_photo(random.choice(YUKI_IMG),"☨ ᴡʜᴇʀᴇ ᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴏᴘᴇɴ ᴛʜᴇ sᴇᴛᴛɪɴɢs ᴍᴇɴᴜ?.",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
