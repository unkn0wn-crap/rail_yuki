...
from YukiBot import DRAGONS, OWNER_ID
    ...
    if not user_id == config.OWNER_ID and user_id in temp:
        ...
        if count >= 7:
            return await message.reply(
                "ɢᴏ sʟᴇᴇᴘ, ʏᴏᴜ ᴀʟʀᴇᴀᴅʏ ɢᴇɴᴇʀᴀᴛᴇᴅ 7 ᴠᴏɪᴄᴇs. ᴄᴏᴍᴇ ʟᴀᴛᴇʀ"
            )
        else:
            temp.append(user_id)
...
async def cb_voice(_, query):
    ...
    if query.from_user.id != user_id:
        return await query.answer('Yᴏᴜ ᴄᴀɴ ɴᴏᴛ ᴀᴄᴄᴇss ᴏᴛʜᴇʀs ʀᴇǫᴜᴇsᴛ')
    ...
    try:
        await query.message.edit(f"**ɢᴇɴᴇʀᴀᴛɪɴɢ {voice_name} ᴡᴀɪᴛ**")
        ...
        await bot.send_audio(
            chat_id=chat_id,
            audio=path,
            caption=f'**Rᴇǫᴜᴇsᴛᴇᴅ ʙʏ {query.from_user.mention}**',
