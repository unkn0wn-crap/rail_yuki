...
# from YukiBot.modules.sql import warns_sql as warnssql
import YukiBot.modules.sql.blacklist_sql as blacklistsql

# from YukiBot.modules.sql import cust_filters_sql as filtersql
# import YukiBot.modules.sql.welcome_sql as welcsql
import YukiBot.modules.sql.locks_sql as locksql
import YukiBot.modules.sql.notes_sql as sql
...
from YukiBot.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("YukiBot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("YukiBot{}.backup".format(chat_id), "rb"),
        caption="❍ *sᴜᴄᴄᴇssғᴜʟʟʏ ᴇxᴘᴏʀᴛᴇᴅ ʙᴀᴄᴋᴜᴘ*\n❍ ᴄʜᴀᴛ ➛ `{}`\n❍ ᴄʜᴀᴛ ɪᴅ ➛ `{}`\n❍ ᴏɴ ➛ `{}`\n\n❍ ɴᴏᴛᴇ ➛ ᴛʜɪs `YukiBot-Backup` ᴡᴀs sᴘᴇᴄɪᴀʟʟʏ ᴍᴀᴅᴇ ғᴏʀ ɴᴏᴛᴇs 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("YukiBot{}.backup".format(chat_id))  # Cleaning file
